...
def plot_gradcam_vs_gt(
    ...
    fig.suptitle(f"Grad-CAM vs GT - {image_id}", fontsize=14, fontweight="bold")
    ...
    thresh = np.percentile(gradcam, 95)
    ...
    if len(rows_c) > 0:
        axes[1, 1].scatter(cols_c, rows_c, c="red", s=3, marker="o", label="Grad-CAM contour")

    axes[1, 1].set_title("(d) Overlay: GT (blue) vs Grad-CAM contour (red)")
    axes[1, 1].axis("off")
